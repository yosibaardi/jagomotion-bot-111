...
import { GroupParticipantsUpdate, MessagesUpsert, Solving, runAutoPromo } from "./src/message.js";
		...
		try {
			...
			if (options.api !== undefined) {
				if (typeof options.api !== "number" || options.api < 1 || options.api > apiList.length) {
					return reject(new Error(`[Fetch Error] Parameter { api: ${options.api} } tidak terdaftar.`));
				}
			}
			const apiName = typeof options.api === "number" ? apiList[options.api - 1] : options.name;
			const base = apiName ? global.APIs[apiName] || apiName : (global.jagomotion?.baseUrl || "");
			const apikey = (global.APIKeys && global.APIKeys[base]) || global.jagomotion?.apiKey || "";
...
let mainPath = global.database?.path || global.database?.url || "jagomotion_session";
const isUrl = new RegExp("^(mongodb|mysql|postgres|postgresql)(\\+srv)?://", "i").test(mainPath);
if (!isUrl) {
	const forbiddenPaths = ["temp", "clonebot", "backup", "node_modules"];
	if (forbiddenPaths.includes(mainPath.trim().toLowerCase())) {
		console.log(chalk.redBright(`[WARNING] Folder database "${mainPath}" tidak valid. Menggunakan folder default ("jagomotion_session").`));
		mainPath = "jagomotion_session";
	}
}
...
async function startJagomotionBot() {
	try {
	...
	if (pairingCode && !phoneNumber && !fs.existsSync("./jagomotion_session/creds.json")) {
		fs.rmSync("./jagomotion_session", { recursive: true, force: true });
	...
	const getMessage = async (key) => {
		...
		return {
			conversation: "Halo Saya JagomotionBot",
		};
	};
		...
		if ((connection === "connecting" || !!qr) && pairingCode && phoneNumber && !sock.authState.creds.registered && !pairingStarted) {
			...
			setTimeout(async () => {
				try {
					...
					let code = await sock.requestPairingCode(phoneNumber, "JAGO" + randomPart);
		...
		if (connection == "open") {
			...
			if (global.db?.set[botNumber] && !global.db?.set[botNumber]?.join) {
				if (global.my?.ch && global.my.ch.length > 0 && global.my.ch.includes("@newsletter")) {
					await sock.newsletterMsg(global.my.ch, { type: "follow" }).catch((e) => {});
	...
	cron.schedule(
		...
		async () => {
			...
			if (global.db?.set[botNumber]?.autobackup) {
				...
				if (new RegExp("^(mongodb|mysql|postgres)", "i").test(dbPath)) {
